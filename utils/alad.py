...
class ALAD:
    ...
    def __init__(
        ...
        params_ = (
            list(self.dis_xz.parameters())
            + list(self.dis_zz.parameters())
            + list(self.dis_xx.parameters())
        )
        self.d_optimizer = optimizer(
            params_, lr=optimizer_kwargs["lr"], betas=(0.5, 0.999)
        )

        self.BCEWithLogits = nn.BCEWithLogitsLoss().to(self.device)
        self.BCEWithLogitsReductionNone = nn.BCEWithLogitsLoss(reduction="none").to(
            self.device
        )
    ...
    def _forward(self, x):
        ...
        z = Variable(
            Tensor(np.random.normal(0, 1, (x.shape[0], self.latent_dim))),
            requires_grad=False,
        ).to(self.device)
        ...
        # discriminator xz
        loss_dis_enc = self.BCEWithLogits(
            out_dis_x_z_gen, torch.ones_like(out_dis_x_z_gen)
        )
        loss_dis_gen = self.BCEWithLogits(
            out_dis_xgen_z, torch.zeros_like(out_dis_xgen_z)
        )
        ...
        # decoder and encoder
        gen_loss_xz = self.BCEWithLogits(
            out_dis_xgen_z, torch.ones_like(out_dis_xgen_z)
        )
        enc_loss_xz = self.BCEWithLogits(
            out_dis_x_z_gen, torch.zeros_like(out_dis_x_z_gen)
        )

        x_real_gen = self.BCEWithLogitsReductionNone(
            x_logit_real, torch.zeros_like(x_logit_real)
        )
        x_fake_gen = self.BCEWithLogitsReductionNone(
            x_logit_fake, torch.ones_like(x_logit_fake)
        )
        z_real_gen = self.BCEWithLogitsReductionNone(
            z_logit_real, torch.zeros_like(z_logit_real)
        )
        z_fake_gen = self.BCEWithLogitsReductionNone(
            z_logit_fake, torch.ones_like(z_logit_fake)
        )
        ...
        loss_generator = gen_loss_xz + cycle_consistency_loss / 2
        loss_encoder = enc_loss_xz + cycle_consistency_loss / 2
    ...
    def step(self, x):
        ...
        if self.verbose:
            self.writer.add_scalar(
                "discriminator_xz loss", dis_loss_xz.item(), self.step_id
            )
            self.writer.add_scalar(
                "discriminator_xx loss", dis_loss_xx.item(), self.step_id
            )
            self.writer.add_scalar(
                "discriminator_zz loss", dis_loss_zz.item(), self.step_id
            )
            self.writer.add_scalar(
                "generator loss", loss_generator.item(), self.step_id
            )
            self.writer.add_scalar("encoder loss", loss_encoder.item(), self.step_id)
            self.writer.add_scalar(
                "cycle consistency loss", cycle_consistency_loss, self.step_id
            )
    ...
    def valid_step(self, x):
        ...
        with torch.no_grad():
            ...
            self.writer.add_scalar(
                "valid_discriminator_xx loss", dis_loss_xx.item(), self.step_id
            )
            self.writer.add_scalar(
                "valid_discriminator_zz loss", dis_loss_zz.item(), self.step_id
            )
            self.writer.add_scalar(
                "valid_cycle_consistency_loss", cycle_consistency_loss, self.step_id
            )
            self.writer.add_scalar(
                "valid_generator_loss", loss_generator.item(), self.step_id
            )
            self.writer.add_scalar(
                "valid_encoder_loss", loss_encoder.item(), self.step_id
            )

    def train(self):
        if self.dataset == "CIFAR10":
            # Each label should be one time the normal sample whereas all others are anomalous
            for normal_i, normal_class in enumerate(self.classes):
                ...
                print(
                    "-" * 24,
                    f"Start training with class {normal_class} as normal class",
                    "-" * 24,
                )
                print("-" * 100)
                ...
                for epoch_id in tqdm(range(self.max_epochs)):
                    ...
                    for (data, _) in self.train_loader[normal_class]:
                        ...
                        if (
                            self.valid_loader[normal_class] is not None
                            and self.step_id % self.report_interval == 0
                        ):
                            try:
    ...
    def evaluate(self):
        if self.dataset == "CIFAR10":
            ...
            for normal_i, normal_class in enumerate(self.classes):
                print(
                    "Number of anomalous samples: {}".format(
                        self.number_anomalous_samples[normal_class]
                    )
                )
            ...
            print(
                "Number of anomalous samples: {}".format(self.number_anomalous_samples)
            )
            self.load_networks()
            self.encoder.eval()
            self.decoder.eval()
    ...
    def plot_roc(self, y_scores, y_labels):
        # Initialize plot of ROC curves for each class individually
        lw = 1
        fig, ax = plt.subplots()
        ...
        for normal_i, normal_class in enumerate(self.classes):
            ...
            print(
                "ROC AUC score of class {}: {:.2f}".format(
                    normal_class, roc_auc[normal_i] * 100
                )
            )
    ...
    def plot_pr(self, y_score, y_true):
        ...
        display = PrecisionRecallDisplay.from_predictions(
            y_true, y_score, name="Features KDDCup99"
        )
