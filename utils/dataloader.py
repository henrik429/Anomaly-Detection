...
def normalize(image):
    ...
    """
    return 2*torch.div(torch.sub(image, torch.min(image)), torch.max(image) - torch.min(image))-1
...
def load_csv(csv_path):
    ...
    categorical_col = ['protocol_type', 'service', 'flag', 'land', 'logged_in', 'is_host_login', 'is_guest_login']
...
def Dataloader(dataset, normal_class=0, batch_size=32):
    ...
    if dataset == "CIFAR10":
        ...
        testset = ds.CIFAR10(root=f'{os.getcwd()}/data/test',
                             train=False,
                             download=True)
        ...
        csv_df = load_csv(train_csv_path)

        # The test data is not labeled. Therefore, we have to split the train set.
...
if __name__ == '__main__':
    ds = Dataloader("CIFAR10")
    ds2 = Dataloader("KDDCup")
